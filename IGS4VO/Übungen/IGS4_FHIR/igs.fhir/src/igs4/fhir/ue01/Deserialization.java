...
public class Deserialization {
	...
	public static void main(String[] args) {
		...
		// TODO: Implement me
		//context has some factory methodes
		//takes fhir resources and processes them
		FhirContext ctx = FhirContext.forDstu3();

		//Parser for XML string
		IParser xmlParser = ctx.newXmlParser();

		Patient p = xmlParser.parseResource(Patient.class, msgString);

		//get the patient identifier
		String patientId = p.getIdentifier().get(0).getValue();

		//family name
		String familyName = p.getName().get(0).getFamily();

		//gender
		String gender = p.getGender().getDisplay();
